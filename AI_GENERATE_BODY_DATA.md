# AI智能生成测试数据（Body）功能

**版本**：v1.3.6  
**发布日期**：2024-12-18  
**功能类型**：🤖 AI增强

---

## 🎯 功能概述

在创建自动化用例时，系统现在会使用AI根据测试用例的具体信息（标题、描述、步骤等）智能生成测试数据（caseDefine中的body部分），而不是使用模板中的空数据。

---

## 🚀 核心优势

### 之前的问题

```
❌ body数据从模板复制
❌ body数据为空或不相关
❌ 需要手动填写测试数据
❌ 测试数据与用例内容不匹配
```

### 现在的解决方案

```
✅ AI根据用例信息智能生成
✅ 测试数据符合业务逻辑
✅ 自动生成1-3条测试数据
✅ 数据与字段定义完全匹配
✅ 真实、合理的测试场景
```

---

## 🔄 完整流程

### 原有流程（v1.3.5.1）

```
1. 获取场景的用例列表
   ↓
2. AI选择最佳模板用例
   ↓
3. 获取模板用例详情（包含header和body）
   ↓
4. 直接使用模板的body（可能为空）
   ↓
5. 创建用例和明细
```

### 新流程（v1.3.6）

```
1. 获取场景的用例列表
   ↓
2. AI选择最佳模板用例
   ↓
3. 获取模板用例详情（获取header字段定义）
   ↓
4. 🆕 AI根据测试用例信息生成测试数据（body）
   ↓
5. 创建用例和明细（包含AI生成的body）
```

---

## 🧠 AI生成逻辑

### 输入信息

AI会综合以下信息生成测试数据：

#### 1. 测试用例信息
- **标题**：测试用例的名称
- **描述**：详细描述
- **前置条件**：执行前的准备工作
- **测试步骤**：具体的操作步骤
- **预期结果**：期望的输出
- **测试类型**：功能测试、性能测试等
- **优先级**：P0, P1, P2等

#### 2. 字段定义（header）
- **字段名**（row）：如 `CP_accidentReason_1`
- **字段显示名**（rowName）：如 "理赔_出险原因_1"
- **字段类型**（type）：数据类型
- **标识**（flag）：特殊标记

#### 3. 环节信息（circulation）
- **环节名称**：如 "理赔"
- **变量组**（vargroup）：如 "CP"

### AI Prompt 结构

```
你是一个自动化测试专家。请根据以下测试用例信息和字段定义，生成1-3条合理的测试数据。

【测试用例信息】
标题：XXX
描述：XXX
前置条件：XXX
测试步骤：XXX
预期结果：XXX
测试类型：XXX
优先级：XXX
环节信息：XXX

【字段定义】
- 字段显示名 (字段名: xxx, 类型: xxx, 标识: xxx)
- ...

【要求】
1. 根据测试用例的具体内容，生成符合字段要求的测试数据
2. 测试数据要真实、合理、符合业务逻辑
3. 生成1-3条测试数据
4. 字段值要符合字段类型和业务含义
5. 日期格式使用 YYYYMMDD
6. 代码类字段要使用正确的业务代码

请以JSON格式返回...
```

### 输出格式

AI会生成符合以下格式的JSON数据：

```json
[
    {
        "casedesc": "测试场景描述",
        "var": {
            "Cont_contno": "IP3713202500007096",
            "Risk_riskcode": "ADDTAE",
            "CP_accidentType_1": "被保人",
            "CP_accidentReason_1": "1-疾病",
            "CP_claimType_1": "02-身故",
            "CP_accidentDay_1": "20250120"
        }
    },
    {
        "casedesc": "另一个测试场景",
        "var": {
            // 另一组测试数据
        }
    }
]
```

系统会将其转换为完整的body格式：

```json
{
    "casezf": "1",
    "casedesc": "测试场景描述",
    "var": { /* 字段值 */ },
    "hoperesult": "成功结案",
    "iscaserun": false,
    "caseBodySN": 1
}
```

---

## 📋 日志输出示例

### 步骤4：AI生成测试数据

```bash
[INFO] 步骤4: 使用AI根据测试用例信息生成测试数据
[INFO] 调用AI生成测试数据（基于16个字段）

[DEBUG] ========== AI Prompt 开始 ==========
你是一个自动化测试专家。请根据以下测试用例信息和字段定义，生成1-3条合理的测试数据。

【测试用例信息】
标题：验证理赔身故赔付
描述：验证被保人因疾病身故时的理赔流程
前置条件：保单已生效
测试步骤：1. 提交理赔申请...
预期结果：成功结案
测试类型：功能测试
优先级：P1
环节信息：
- 理赔 (CP)

【字段定义】
- 保单信息_保单号 (字段名: Cont_contno)
- 险种_险种编码 (字段名: Risk_riskcode, 标识: RiskFlag)
- 理赔_出险人类型_1 (字段名: CP_accidentType_1)
- 理赔_出险原因_1 (字段名: CP_accidentReason_1)
- 理赔_理赔类型_1 (字段名: CP_claimType_1)
...

【要求】
1. 根据测试用例的具体内容，生成符合字段要求的测试数据
2. 测试数据要真实、合理、符合业务逻辑
...

请以JSON格式返回...
[DEBUG] ========== AI Prompt 结束 ==========

[DEBUG] ========== AI Response 开始 ==========
[
    {
        "casedesc": "疾病身故理赔-正常场景",
        "var": {
            "Cont_contno": "IP3713202500007096",
            "Risk_riskcode": "ADDTAE",
            "CP_accidentType_1": "被保人",
            "CP_accidentReason_1": "1-疾病",
            "CP_claimType_1": "02-身故",
            "CP_cct_1": "1",
            "CP_clmInsBnftECD_1": "ADDTAE4001",
            "CP_StmSrcCd_1": "02-个险",
            "CP_isCheck_1": "N",
            "CP_accidentDay_1": "20250120"
        }
    },
    {
        "casedesc": "意外身故理赔",
        "var": {
            "Cont_contno": "IP3713202500106628",
            "Risk_riskcode": "ADDTAE",
            "CP_accidentType_1": "被保人",
            "CP_accidentReason_1": "2-意外",
            "CP_claimType_1": "02-身故",
            "CP_cct_1": "1",
            "CP_clmInsBnftECD_1": "ADDTAE4001",
            "CP_StmSrcCd_1": "02-个险",
            "CP_isCheck_1": "N",
            "CP_accidentDay_1": "20250731"
        }
    }
]
[DEBUG] ========== AI Response 结束 ==========

[INFO] ✅ AI生成了 2 条测试数据
[INFO] ✅ 已将AI生成的 2 条测试数据添加到caseDefine
```

### 步骤5：创建用例和明细

```bash
[INFO] 步骤5: 一次性创建用例和明细
[INFO] 使用模板的circulation信息: [{'num': 2, 'name': '理赔', 'vargroup': 'CP'}]
[INFO] ✅ caseDefine 已添加: 16 个字段(header), 2 个测试数据(body)
[INFO] 调用自动化平台创建用例和明细
[INFO] URL: http://localhost:8087/ai/case/createCaseAndBody
[INFO] 用例名称: 验证理赔身故赔付
[INFO] Payload keys: ['name', 'moduleId', 'sceneId', 'scenarioType', 'description', 'tags', 'nodePath', 'type', 'project', 'sceneIdModule', 'circulation', 'caseDefine']
[INFO] Circulation: 1 个环节
[INFO] CaseDefine: header=16, body=2
[INFO] 响应状态码: 200
[INFO] 用例和明细创建成功
```

---

## 🎨 生成的测试数据特点

### 1. 业务合理性

- **保单号格式**：`IP + 分公司代码 + 年份 + 流水号`
- **日期格式**：`YYYYMMDD`（如 20250120）
- **业务代码**：使用真实的业务编码（如 1-疾病、02-身故）

### 2. 多样性

AI会根据测试用例的描述生成多个场景：
- 主要测试场景（正常流程）
- 边界场景
- 异常场景（如适用）

### 3. 完整性

每条测试数据包含：
- **casedesc**：清晰的场景描述
- **var**：所有必需字段的值
- **自动填充字段**：casezf, hoperesult, iscaserun, caseBodySN

---

## 🔍 调试和验证

### 查看AI调用详情

在后端日志中查找：
```bash
[DEBUG] ========== AI Prompt 开始 ==========
```

可以看到：
1. **发送给AI的完整提示词**：包含所有测试用例信息和字段定义
2. **AI的原始响应**：JSON格式的测试数据
3. **解析结果**：生成的测试数据条数

### 验证生成的数据

检查以下内容：
1. ✅ body数组不为空
2. ✅ 每条数据包含casedesc和var
3. ✅ var中的字段名与header中的定义匹配
4. ✅ 字段值符合业务规则

---

## ⚠️ 异常处理

### AI服务不可用

```bash
[WARNING] AI服务不可用，返回空body
```

**原因**：无法连接到AI服务或数据库配置缺失

**解决**：检查AI服务配置和数据库连接

### JSON解析失败

```bash
[ERROR] 解析AI返回的JSON失败: ...
[DEBUG] AI返回内容: ...
```

**原因**：AI返回的不是有效的JSON格式

**解决**：AI服务会自动降级，返回空body，用例仍可正常创建

### 缺少必要信息

```bash
[WARNING] ⚠️ 缺少必要信息，无法生成测试数据
[WARNING] 缺少header字段定义
```

**原因**：模板用例没有字段定义

**解决**：选择包含完整字段定义的模板用例

---

## 📊 功能对比

| 特性 | v1.3.5.1及之前 | v1.3.6（当前） |
|------|---------------|---------------|
| **body来源** | 模板复制 | AI智能生成 |
| **数据相关性** | 可能不相关 | 完全匹配测试用例 |
| **数据质量** | 依赖模板质量 | AI保证业务合理性 |
| **数据多样性** | 单一 | 1-3条不同场景 |
| **手动工作** | 需要后续编辑 | 开箱即用 |
| **调试信息** | 简单 | 详细的prompt和response日志 |

---

## 🚀 使用步骤

### 1. 确保配置正确

- ✅ AI服务配置已设置（数据库或环境变量）
- ✅ 自动化平台API地址已配置
- ✅ 模块ID已配置

### 2. 创建测试用例

在测试用例管理中创建新用例，填写：
- **标题**：清晰明确
- **描述**：详细的业务场景
- **测试步骤**：具体的操作步骤
- **预期结果**：期望的输出

### 3. 点击"自动化"按钮

系统会自动：
1. 匹配最佳场景
2. 选择最佳模板用例
3. 提取字段定义（header）
4. 🆕 **AI生成测试数据（body）**
5. 创建完整的自动化用例

### 4. 查看结果

在成功提示中可以看到：
- 匹配的场景信息
- 使用的模板用例
- **生成的测试数据条数**

---

## 💡 最佳实践

### 1. 编写高质量的测试用例

AI生成的数据质量取决于测试用例信息的完整性：

**好的例子**：
```
标题：验证理赔身故赔付金额计算
描述：在理赔审核环节，验证系统对BANTBC身故保险金的计算逻辑（取已交保费与身故当日现金价值的较大者）
前置条件：保单已承保，保单号为IP3713202500007096
测试步骤：
1. 登录理赔系统
2. 选择被保人身故理赔
3. 输入出险原因为疾病
4. 提交理赔申请
预期结果：系统计算赔付金额正确，理赔流程成功结案
```

**不好的例子**：
```
标题：测试理赔
描述：理赔测试
```

### 2. 选择高质量的场景

场景应该：
- ✅ 有明确的业务线和渠道
- ✅ 有完整的字段定义
- ✅ 有合理的环节信息

### 3. 验证生成的数据

虽然AI生成的数据通常是合理的，但建议：
- 检查生成的保单号、险种代码等是否符合实际业务
- 验证日期格式和时间逻辑
- 确认关键字段的值

---

## 🔗 相关文档

- [用例数据结构说明](./CASE_DATA_STRUCTURE.md)
- [AI智能模板匹配](./AI_TEMPLATE_MATCHING.md)
- [自动化平台集成](./AUTOMATION_PLATFORM_INTEGRATION.md)
- [v1.3.5.1 caseDefine修复](./FIX_CASEDEFINE_MISSING.md)

---

## 📝 技术实现

### 核心方法

**文件**：`backend/app/services/automation_service.py`

```python
def generate_case_body_by_ai(
    self,
    header_fields: List[Dict[str, Any]],
    test_case_info: Dict[str, Any],
    circulation: List[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    """使用AI根据字段定义和测试用例信息生成测试数据(body)"""
    # 1. 构建字段描述
    # 2. 构建AI提示词
    # 3. 调用AI服务
    # 4. 解析JSON响应
    # 5. 转换为完整的body格式
    # 6. 返回生成的测试数据
```

### 集成点

在 `create_case_with_fields` 方法的第4步：

```python
# 第四步：使用AI生成测试数据（body）
generated_body = self.generate_case_body_by_ai(
    header_fields=header_fields,
    test_case_info=test_case_info,
    circulation=circulation
)

# 将生成的body添加到case_detail中
case_detail['caseDefine']['body'] = generated_body
```

---

**版本**：v1.3.6  
**发布日期**：2024-12-18  
**下一步计划**：支持更复杂的测试数据生成场景


