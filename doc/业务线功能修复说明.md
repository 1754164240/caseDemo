...
创建了 `backend/scripts/update_test_point_prompt.py` 脚本,用于更新数据库中的 Prompt 配置。
...
```bash
cd backend
python -m scripts.update_test_point_prompt
```
...
执行 `python -m scripts.test_business_line` 进行验证:
...
- `backend/scripts/update_test_point_prompt.py` - Prompt 更新脚本
- `backend/scripts/test_business_line.py` - 业务线功能测试脚本
...
4. **测试业务线识别**:
```bash
python -m scripts.test_business_line
```
