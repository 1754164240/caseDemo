...
**Q: 下载的文件打不开？**
- 确认文件扩展名正确
- 使用对应的软件打开（Word、PDF 阅读器等）
- 检查文件是否损坏

**Q: 中文文件名下载后显示乱码？**
- ✅ 已修复：前后端协同处理中文文件名
- ✅ 后端：使用 RFC 5987 标准的 `filename*` 参数（UTF-8 编码）
- ✅ 前端：正确解析 `Content-Disposition` 响应头并 URL 解码
- ✅ 兼容所有现代浏览器（Chrome、Firefox、Edge、Safari）
- ✅ 示例：`新产品理赔分析V1.0.xlsx` 会正确显示

**技术实现**:
- 后端：`filename*=UTF-8''%E6%96%B0%E4%BA%A7%E5%93%81.xlsx`
- 前端：使用 `decodeURIComponent()` 解码文件名
- 回退机制：提供 ASCII 版本的 filename 参数

---

### 19. 需求重新生成测试点后状态更新

**问题**: 需求重新生成测试点成功后，状态没有从"失败"改成"已完成"

**原因**:
- 后台任务 `regenerate_test_points_background` 没有更新需求状态
- 只生成了测试点，但没有将需求状态设置为 `COMPLETED`

**解决方案**:

已在后端 `test_points.py` 中修复：
...
1. **查看后端日志**:
   ```
   [INFO] 开始重新生成测试点，需求 ID: 1
   [INFO] 文档解析成功，文本长度: 1234
   [INFO] 成功生成 5 个测试点
   [INFO] 测试点重新生成完成，需求 ID: 1
   ```
...
**注意事项**:
- ✅ 确保 WebSocket 连接正常
- ✅ 查看浏览器控制台是否有 WebSocket 连接成功的日志
- ✅ 如果状态仍未更新，检查后端日志是否有错误
...
**问题**: 点击查看测试用例详情时，浏览器控制台报错：
```
Uncaught Error: Objects are not valid as a React child
(found: object with keys {step, action, expected})
```

**原因**: 测试用例的某些字段（如 `test_steps`）可能是对象数组，不能直接作为 React 子元素渲染。

**数据结构示例**:
```javascript
// test_steps 可能是对象数组
test_steps: [
  { step: 1, action: "打开页面", expected: "页面正常显示" },
  { step: 2, action: "点击按钮", expected: "弹出对话框" }
]
...
**解决方案**: ✅ 已修复 - 智能检测字段类型并正确渲染

**修复内容**:
- ✅ 前置条件：支持字符串、数组、对象类型
- ✅ 测试步骤：支持字符串、字符串数组、对象数组
- ✅ 预期结果：支持字符串、数组、对象类型
- ✅ 对象数组格式化显示（步骤号、操作、预期）
- ✅ 左侧蓝色边框区分不同步骤
